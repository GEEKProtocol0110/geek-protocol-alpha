...
export interface TimingInput {
  ...
  /**
   * The per-question budget for THIS game mode. Quiz allows 15s, Gauntlet 20s.
   * Getting this wrong under-credits honest players, so callers must pass their
   * own value rather than inherit the quiz default.
   */
...
export interface TimingVerdict {
  ok: boolean;
  reason?: string;
  /** Client timings clamped to a sane range; use these, never the raw input. */
  effectiveTimings: number[];
  /** Multiplier applied to speed bonuses so they can't exceed the wall clock. */
  speedCreditFactor: number;
  elapsedSeconds: number;
  claimedSeconds: number;
  flags: string[];
}

export function validateAttemptTiming(input: TimingInput): TimingVerdict {
  const {
    issuedAtMs,
    submittedAtMs,
    questionCount,
    clientTimings,
    maxSecondsPerQuestion = MAX_SECONDS_PER_QUESTION,
  } = input;
  const flags: string[] = [];
  ...
  if (elapsedSeconds < humanFloor) {
    return {
      ok: false,
      reason: `Attempt submitted in ${elapsedSeconds.toFixed(2)}s, below the ${humanFloor.toFixed(
        2
      )}s minimum for ${questionCount} questions.`,
      effectiveTimings,
      speedCreditFactor: 0,
      elapsedSeconds,
      claimedSeconds,
  ...
  const claimedCredit = effectiveTimings.reduce(
    (sum, t) => sum + Math.max(0, maxSecondsPerQuestion - t),
    0
  );
  const allowedCredit = Math.max(
    0,
    questionCount * maxSecondsPerQuestion -
  ...
  if (questionCount >= 4) {
    ...
    if (unique.size === 1) flags.push("uniform_timings");
  }
