...
const num = (env: string, fallback: number) =>
  Math.max(0, parseInt(process.env[env] || String(fallback), 10) || fallback);
...
export async function getReviewEligibility(
  ...
  const accountAgeHours = (Date.now() - user.dateCreated.getTime()) / 3_600_000;
  const accuracyPct = user.reviewAccuracy;

  const reasons: string[] = [];
  if (user.level < MIN_LEVEL) reasons.push(`Requires Level ${MIN_LEVEL} (you are ${user.level}).`);
  if (wins < MIN_WINS) {
    reasons.push(
      `Requires ${MIN_WINS} quiz wins (${WIN_MIN_CORRECT}+ correct). You have ${wins}.`
    );
  }
  ...
  if (resolvedReviews >= ACCURACY_GRACE_REVIEWS && accuracyPct < MIN_ACCURACY_PCT) {
    reasons.push(
      `Review accuracy ${Math.round(accuracyPct)}% is below the ${MIN_ACCURACY_PCT}% minimum.`
    );
  }
...
export async function cappedCreatorIds(
  prisma: PrismaClient,
  userId: number
): Promise<number[]> {
 ...
 */
export async function recomputeReviewerAccuracy(prisma: PrismaClient, validatorId: number) {
  ...
  const agreed = validations.filter(
    (v) =>
      (v.action === "approve" && v.question.status === "approved") ||
      (v.action === "reject" && v.question.status === "rejected")
  ).length;
