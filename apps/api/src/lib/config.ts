...
interface SecretSpec {
  ...
  minLength: number;
  purpose: string;
  /** Must be exactly 32 bytes because it keys AES-256. */
  exactBytes?: number;
}
...
export function inspectConfig(): ConfigProblem[] {
  ...
  for (const spec of REQUIRED_SECRETS) {
    ...
    if (spec.exactBytes && Buffer.byteLength(value, "utf8") !== spec.exactBytes) {
      problems.push({
        level,
        variable: spec.name,
        message: `must be exactly ${spec.exactBytes} bytes (got ${Buffer.byteLength(
          value,
          "utf8"
        )}) — ${spec.purpose}`,
      });
  ...
  if (process.env.JWT_SECRET && !process.env.SECRET_KEY) {
    problems.push({
      ...
      variable: "SECRET_KEY",
      message:
        "JWT_SECRET is set but SECRET_KEY is not. Sessions are signed with SECRET_KEY; " +
  ...
  if (IS_PRODUCTION) {
    if (process.env.DEMO_MODE === "true") {
      problems.push({
        ...
        variable: "DEMO_MODE",
        message:
          "is true in production — this disables wallet signature verification entirely, " +
    ...
    if (process.env.ENABLE_REWARDS === "true") {
      ...
      if (!process.env.GEEK_TOKEN_TICKER) {
        problems.push({
          ...
          message:
            "ENABLE_REWARDS is true but the KRC-20 ticker is not set, so payouts cannot " +
      ...
      if (!process.env.KASPA_TRANSFER_PROVIDER) {
        problems.push({
          ...
          message:
            "ENABLE_REWARDS is true but no on-chain transfer provider is configured. " +
...
export function formatProblems(problems: ConfigProblem[]): string {
  ...
  if (errors.length) {
    ...
    for (const p of errors) lines.push(`   ✗ ${p.variable}: ${p.message}`);
  }
  if (warnings.length) {
    lines.push(`\n  ${warnings.length} configuration warning(s):`);
    for (const p of warnings) lines.push(`   ! ${p.variable}: ${p.message}`);
  }
  return lines.join("\n");
}
 ...
 */
export function assertProductionConfig(log: (msg: string) => void = console.error) {
