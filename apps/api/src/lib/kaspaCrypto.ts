...
export function publicKeyFromAddress(address: string): Uint8Array | null {
  const decoded = decodeKaspaAddress(address);
  if (!decoded || decoded.version !== "schnorr") return null;
...
export function addressFromPrivateKey(
  ...
  if (sk.length !== 32) throw new Error("Private key must be 32 bytes of hex");
...
export function generateKeypair(prefix = networkPrefix()): GeneratedWallet {
...
function isValidScalar(buf: Buffer): boolean {
...
function decodeSignature(signature: string): Uint8Array | null {
  ...
  try {
    ...
    if (buf.length === 65) return Uint8Array.from(buf.subarray(1));
  } catch {
    return null;
  }
...
function sha256(data: Uint8Array | string): Uint8Array {
  return Uint8Array.from(
    crypto.createHash("sha256").update(typeof data === "string" ? Buffer.from(data, "utf8") : data).digest()
  );
}
 ...
 */
function messageDigests(message: string): Uint8Array[] {
...
function encodeVarIntLength(n: number): Buffer {
...
export async function verifyKaspaSignature(
  ...
  for (const digest of messageDigests(message)) {
    try {
      if (schnorr.verify(sig, digest, publicKey)) return true;
    } catch {
