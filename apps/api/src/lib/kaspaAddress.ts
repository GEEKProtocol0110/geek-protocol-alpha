...
export type AddressVersion = "schnorr" | "ecdsa" | "p2sh";

export interface DecodedAddress {
  prefix: string;
  version: AddressVersion;
  /** 32 bytes for schnorr, 33 for ecdsa, 32 for p2sh. */
  payload: Uint8Array;
}

/** CashAddr checksum over GF(2^5), 40-bit polymod. */
function polymod(values: Uint8Array | number[]): bigint {
  let c = 1n;
...
const VERSION_BY_BYTE: Record<number, AddressVersion> = {
  0x00: "schnorr",
  0x01: "ecdsa",
  0x08: "p2sh",
};

const EXPECTED_LENGTH: Record<AddressVersion, number> = {
  schnorr: 32,
  ecdsa: 33,
  p2sh: 32,
};
 ...
 */
export function decodeKaspaAddress(address: string): DecodedAddress | null {
  if (typeof address !== "string" || address.length < 10) return null;
  ...
  if (polymod([...prefixToWords(prefix), ...words]) !== 0n) return null;
  ...
  if (payload.length !== EXPECTED_LENGTH[version]) return null;
...
export function isValidKaspaAddress(address: string, expectedPrefix?: string): boolean {
  ...
  if (expectedPrefix && decoded.prefix !== expectedPrefix.toLowerCase()) return false;
  return true;
}

/** Prefix for the configured network. */
export function networkPrefix(network = process.env.KASPA_NETWORK || "mainnet"): string {
  const n = network.toLowerCase();
  if (n.startsWith("test")) return "kaspatest";
  if (n.startsWith("dev")) return "kaspadev";
  if (n.startsWith("sim")) return "kaspasim";
  return "kaspa";
}

export function encodeKaspaAddress(
  payload: Uint8Array,
  version: AddressVersion = "schnorr",
  prefix = networkPrefix()
): string {
  const versionByte =
    version === "schnorr" ? 0x00 : version === "ecdsa" ? 0x01 : 0x08;

  const dataWords = convertBits([versionByte, ...payload], 8, 5, true);
  if (!dataWords) throw new Error("Failed to encode Kaspa address payload");
