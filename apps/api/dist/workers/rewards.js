...
function readBigIntEnv(key, fallback) {
...
function scheduleConfirmation(rewardId, attemptId, txid) {
    setTimeout(async () => {
        try {
...
async function processRewardJob(job) {
    ...
    try {
        ...
        if (attempt.scorePct < MIN_SCORE_FOR_REWARD) {
            await markFailedReward(attempt.id, user.id, `Score ${attempt.scorePct}% below threshold ${MIN_SCORE_FOR_REWARD}%`);
        ...
        if (attempt?.user) {
            try {
                await markFailedReward(attempt.id, attempt.user.id, err instanceof Error ? err.message : "Unknown worker error");
            }
            catch (markErr) {
...
export async function startRewardWorker() {
    ...
    while (true) {
        try {
            ...
            if (data && data[1]) {
                try {
                    const jobCandidate = JSON.parse(String(data[1]));
...
function isRewardJob(candidate) {
    return Boolean(candidate && typeof candidate.attemptId === "string");
}
