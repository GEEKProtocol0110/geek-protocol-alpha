...
async function main() {
    ...
    try {
        ...
        if (questions === 0)
            warn("no approved questions — quizzes will have nothing to serve");
    }
    catch (err) {
        bad(`PostgreSQL unreachable: ${err.message}`);
    }
    console.log("\nRedis");
    ...
    try {
        ...
        if (pong === "PONG")
            ...
            bad(`unexpected Redis reply: ${pong}`);
        ...
        bad(`Redis unreachable: ${err.message}`);
    }
    console.log("\nPayout pipeline");
    ...
    try {
        ...
        if (isValidKaspaAddress(w.address, prefix))
            ok(`keypair generation works (${prefix})`);
        else
            bad("generated address failed its own validation");
    }
    catch (err) {
        bad(`keypair generation failed: ${err.message}`);
    }
    if (process.env.DEMO_MODE === "true") {
        if (IS_PRODUCTION)
            bad("DEMO_MODE=true in production disables wallet signature verification");
        else
            warn("DEMO_MODE=true — wallet signatures are NOT verified");
    }
    else {
