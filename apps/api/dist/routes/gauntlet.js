...
export async function gauntletRoutes(fastify) {
    ...
    fastify.post("/run/:runId/round/:round/questions", async (request, reply) => {
        ...
        const topics = JSON.parse(run.selectedTopics ?? JSON.stringify(TOPICS));
        const activeState = parseState(run.activeState);
        const paidRounds = activeState.paidRounds ?? [];
    ...
    fastify.post("/run/:runId/round/:round/submit", async (request, reply) => {
        ...
        if (!timing.ok) {
            fastify.log.warn({ userId, runId, round, reason: timing.reason, flags: timing.flags }, "Gauntlet submission rejected by timing validation");
        ...
        const results = effectiveAnswers.map((ans, i) => {
            ...
            const speedBonus = isCorrect && fullCredit && timeTaken < 5
                ? 0.2
                : isCorrect && fullCredit && timeTaken < 9
                    ? 0.1
                    : 0;
            let reward = isCorrect ? cfg.rewardPerCorrect : 0;
            if (modifier === "double_down")
                reward *= 2;
            if (modifier === "hot_streak" && i < 5 && answers.slice(0, i + 1).every((a, idx) => a === correctAnswers[idx]))
                reward *= 1.5;
        ...
        let refund = 0;
        if (modifier === "safety_net" && correctCount < 5)
            refund = Math.round(cfg.fee * 0.5);
        // Persist attempts
        try {
    ...
    fastify.post("/run/:runId/cashout", async (request, reply) => {
        ...
        const totalGeek = run.totalGeekEarned;
        await fastify.prisma.gauntletRun.update({
            where: { id: runId },
