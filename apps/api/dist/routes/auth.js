...
export async function authRoutes(fastify) {
    ...
    fastify.post("/nonce", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
        const parse = NonceSchema.safeParse(req.body);
        if (!parse.success) {
            return reply.code(400).send({ error: parse.error.flatten() });
        }
        const { nonce, message, expiresAt, expiresInMs } = await issueNonce(fastify.redis, parse.data.walletAddress);
    ...
    fastify.post("/wallet-login", async (req, reply) => {
        ...
        if (!consumed.ok) {
            logger.warn({ walletAddress, reason: consumed.reason }, "Wallet login rejected: nonce already used or expired");
