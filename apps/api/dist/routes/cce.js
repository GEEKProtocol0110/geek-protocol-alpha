...
import { APPROVAL_THRESHOLD, REJECTION_THRESHOLD, MIN_LEVEL, MIN_REVIEW_SECONDS, REVIEW_REWARD_GEEK, cappedCreatorIds, getReviewEligibility, recomputeReviewerAccuracy, } from "../lib/reviewIntegrity";
...
export async function cceRoutes(fastify) {
    ...
    fastify.get("/review/next", { preHandler: fastify.authenticate }, async (req, reply) => {
        ...
        if (candidates === 0)
            return reply.send({ data: null, eligibility });
    ...
    fastify.post("/review", { preHandler: fastify.authenticate }, async (req, reply) => {
        ...
        if (question.createdBy != null && blockedCreators.includes(question.createdBy)) {
            logger.warn({ userId, creatorId: question.createdBy, questionId }, "Review blocked: weekly per-creator cap reached");
        ...
        if (resolved) {
            ...
            await Promise.all(uniqueVoters.map((id) => recomputeReviewerAccuracy(fastify.prisma, id)));
        }
