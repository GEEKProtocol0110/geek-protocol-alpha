...
// ── Route plugin ──────────────────────────────────────────────────────────────
export async function tokenRoutes(fastify) {
    ...
    fastify.post("/buy", { preHandler: fastify.authenticate }, async (req, reply) => {
        const parse = z.object({ kasAmount: z.number().positive().max(100000) }).safeParse(req.body);
        if (!parse.success)
            return reply.code(400).send({ success: false, error: "Invalid amount" });
        const { kasAmount } = parse.data;
        const geekOut = kasToGeek(kasAmount);
        if (geekOut <= 0)
            return reply.code(400).send({ success: false, error: "Insufficient liquidity" });
    ...
    fastify.post("/sell", { preHandler: fastify.authenticate }, async (req, reply) => {
        const parse = z.object({ geekAmount: z.number().positive().max(1000000) }).safeParse(req.body);
        if (!parse.success)
            return reply.code(400).send({ success: false, error: "Invalid amount" });
    ...
    fastify.post("/trade-sticker", { preHandler: fastify.authenticate }, async (req, reply) => {
        const parse = z.object({ userStickerId: z.number().int().positive() }).safeParse(req.body);
        if (!parse.success)
            return reply.code(400).send({ success: false, error: "Invalid sticker id" });
        const userId = req.jwtUser.userId;
    ...
    fastify.post("/add-liquidity", { preHandler: fastify.authenticate }, async (req, reply) => {
        const parse = z.object({ kasAmount: z.number().positive().max(100000) }).safeParse(req.body);
        if (!parse.success)
            return reply.code(400).send({ success: false, error: "Invalid amount" });
        const { kasAmount } = parse.data;
