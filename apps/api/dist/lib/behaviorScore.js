...
export const BehaviorSignalsSchema = z.object({
    ...
    pointerSamples: z.number().int().min(0).max(100000).default(0),
    /** Sum of segment lengths, px. Straight-line teleports keep this near zero. */
    pointerPathLength: z.number().min(0).max(10000000).default(0),
    /** Straight-line distance between first and last pointer position, px. */
    pointerNetDisplacement: z.number().min(0).max(1000000).default(0),
    /** Std-dev of inter-sample intervals, ms. Synthetic input is metronomic. */
    pointerIntervalStdDev: z.number().min(0).max(100000).default(0),
    /** Direction changes — humans overshoot and correct. */
    directionChanges: z.number().int().min(0).max(100000).default(0),
    keyboardEvents: z.number().int().min(0).max(100000).default(0),
    touchEvents: z.number().int().min(0).max(100000).default(0),
...
export function scoreBehavior(signals) {
    ...
    const pointerRelevant = signals.inputModality === "mouse" ||
