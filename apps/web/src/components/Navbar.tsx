...
export function Navbar() {
  ...
  return (
    <>
      {/* Mobile drawer */}
      <div className={`fixed top-0 left-0 w-full h-full bg-white z-[500] transform transition-transform duration-300 overflow-y-auto p-8 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex justify-between items-center pb-6 border-b border-[var(--border-soft)] mb-6">
          <span className="font-extrabold tracking-tight text-lg text-[var(--brand-primary)]">GEEK PROTOCOL</span>
      ...
      {/* Main nav */}
      <nav className="sticky top-0 z-[200] bg-white/90 backdrop-blur-md border-b border-[var(--border-soft)]">
        <div className="flex items-center justify-between px-4 md:px-8 py-3 max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center gap-4">
            <a href="/" className="flex items-center gap-2">
              ...
              <span className="font-extrabold text-sm text-[var(--text-1)] hidden sm:block">Geek Protocol</span>
            </a>
            <span className="badge-pill hidden sm:inline-flex text-[var(--brand-secondary)]">
          ...
          <div className="hidden md:flex items-center gap-6">
            <div className="flex items-center gap-1">
              {isAuthenticated && (
                <a href="/dashboard" className="rounded-full px-4 py-1.5 text-[var(--text-2)] hover:text-[var(--brand-primary)] hover:bg-[var(--surface-2)] transition text-sm font-semibold">Dashboard</a>
              )}
              <a href="/play"        className="rounded-full px-4 py-1.5 text-[var(--text-2)] hover:text-[var(--brand-primary)] hover:bg-[var(--surface-2)] transition text-sm font-semibold">Play</a>
              <a href="/leaderboard" className="rounded-full px-4 py-1.5 text-[var(--text-2)] hover:text-[var(--brand-primary)] hover:bg-[var(--surface-2)] transition text-sm font-semibold">Leaderboard</a>
              <a href="/stickers"    className="rounded-full px-4 py-1.5 text-[var(--text-2)] hover:text-[var(--brand-primary)] hover:bg-[var(--surface-2)] transition text-sm font-semibold">Stickers</a>
              <a href="/token"      className="rounded-full px-4 py-1.5 text-[var(--text-2)] hover:text-[var(--brand-primary)] hover:bg-[var(--surface-2)] transition text-sm font-semibold">$GEEK</a>
              <a href="/litepaper"   className="rounded-full px-4 py-1.5 text-[var(--text-2)] hover:text-[var(--brand-primary)] hover:bg-[var(--surface-2)] transition text-sm font-semibold">Litepaper</a>
            </div>
            ...
            {isAuthenticated && user ? (
              <div className="flex items-center gap-2">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-2 bg-white rounded-2xl border border-[var(--border-soft)] shadow-[var(--shadow-soft)] min-w-[160px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleLogin} className="text-sm text-[var(--text-2)] font-semibold hover:text-[var(--brand-primary)] transition px-3 py-1.5">
                  Sign In
                </button>
                <button
          ...
          {/* Mobile hamburger */}
          <button onClick={() => setMobileOpen(true)} className="md:hidden text-[var(--text-2)] rounded-full border border-[var(--border-soft)] px-2 py-1.5">
            <FaBars />
          </button>
        </div>
      </nav>
