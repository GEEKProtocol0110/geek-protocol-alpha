        ...
        <motion.div variants={container} initial="hidden" animate="show" className="flex min-w-0 flex-col items-start">
          ...
          <motion.h1 variants={item} className="gp-wordmark mt-5 w-full leading-none sm:mt-6">
            {/* Fluid clamp() sizing — scales continuously with viewport width
               ...
               The lower bound has to stay genuinely small: "PROTOCOL" is the
               longest line, and at the old 4.9rem floor it rendered 443px wide
               on every phone — the clamp never reached its own preferred value
               below ~430px of viewport, so the L was cut off the screen. */}
            <span className="block origin-left -skew-x-[8deg] text-[clamp(2.5rem,-0.5rem+16vw,7.15rem)] gp-wordmark-cyan lg:text-[clamp(3.8rem,1.3rem+3.9vw,6.2rem)]">GEEK</span>
            <span className="mt-0 block origin-left -skew-x-[8deg] text-[clamp(2.5rem,-0.5rem+16vw,7.15rem)] gp-wordmark-pink lg:text-[clamp(3.8rem,1.3rem+3.9vw,6.2rem)]">PROTOCOL</span>
          </motion.h1>
          ...
          <motion.div variants={item} className="mt-6 flex flex-row flex-nowrap items-center gap-3 sm:gap-[1.3rem]">
            <Link
              href="/auth/register"
              className="inline-flex items-center gap-[0.65rem] rounded-2xl border-[2.6px] border-[var(--ink)] bg-[var(--gp-pink)] px-5 py-3 text-base sm:px-[2.6rem] sm:py-[1.3rem] sm:text-[1.3rem] font-bold text-white shadow-[6.5px_6.5px_0px_0px_var(--gp-pink-dark)] transition-[transform,box-shadow] duration-100 hover:-translate-x-[2.6px] hover:-translate-y-[2.6px] hover:shadow-[9.1px_9.1px_0px_0px_var(--gp-pink-dark)] active:translate-x-[5.2px] active:translate-y-[5.2px] active:shadow-none"
            >
              Get Started →
            </Link>
            <Link
              href="/auth/login"
              className="inline-flex items-center rounded-2xl border-[2.6px] border-[var(--border-soft)] bg-transparent px-5 py-3 text-base sm:px-[2.6rem] sm:py-[1.3rem] sm:text-[1.3rem] font-bold text-white transition-colors duration-100 hover:border-[var(--gp-cyan)] hover:text-[var(--gp-cyan)]"
            >
              Sign In
            </Link>
          </motion.div>
